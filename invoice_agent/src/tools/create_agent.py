...
from src.tools.parse_function import XMLAgentOutputParser
...
def format_xml(
    ...
    for action, observation in intermediate_steps:
        log += (
            f"<function>{action.tool}</function><parameters>{action.tool_input}"
            f"</parameters><function_result>{observation}</function_result>"
        )
    return log
        ...
        **kwargs: Any,
        ...
        for action, observation in intermediate_steps:
            log +=  (
                "<function_results>\n"
                + '\n'.join(
                    f"<result>\n<function_name>{action.tool}</function_name>\n<stdout>\n{observation}\n</stdout>\n</result>" 
                ) + "\n</function_results>"
            )
        tools = ""
        ...
        inputs = {
            ...
            "stop": ["</function_calls>", "</final_answer>", "</ask_user>"],
        }
    ...
    async def aplan(
        ...
        **kwargs: Any,
        ...
        inputs = {
            ...
            "tools": tools,
            "question": kwargs["input"],
            "stop": ["</tool_input>", "</final_answer>","</ask_user>"],
        }
...
def create_xml_agent(
    ...
    *,
    ...
    if stop_sequence:
        stop = ["</function_result>", "</ask_user>", "</final_answer>"] if stop_sequence is True else stop_sequence
        llm_with_stop = llm.bind(stop=stop)
    else:
        llm_with_stop = llm

    agent = (
        # RunnableWithMessageHistory.assign(
